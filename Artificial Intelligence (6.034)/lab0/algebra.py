...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        ...
        for factor1 in expr1:
            for factor2 in expr2:
                factors.append(Product([factor1, factor2]))
 
        result = Sum(factors)
    

    if (isinstance(expr1, Sum) and isinstance(expr2, Product)) or (isinstance(expr1, Product) and isinstance(expr2, Sum)):
        ...
        for term in sum_expr:
            new_list = list(prod_expr)
            new_list.append(term)
            factors.append(Product(new_list).simplify())
    ...
    if isinstance(expr1, Product) and isinstance(expr2, Product):
        ...
        result = Product(list(expr1) + list(expr2))

    print result
    return result


def reduce_factors(exp):
    non_exps = []
    exps = []

    if not isinstance(exp, Expression): return exp
    
    # Separate expressions from non-expressions
    for term in exp:
        if isinstance(term, int):
            non_exps.append(term)
        else:
            exps.append(term)

    # Simplify by adding or multiplying the non-expressions
    if isinstance(exp, Sum):
        reduction = 0
        for i in non_exps: reduction += i
    else:
        reduction = 1
        for i in non_exps: reduction *= i
    ...
    if len(exps) > 0:
        if reduction > 0: exps.append(reduction)
        if isinstance(exp, Sum): return Sum(exps).simplify()
        return Product(exps)
    else:
...
##    print reduce_factors(Product([3,8]))

##    result = do_multiply( Sum([1, 2]), Sum( [Sum([3, 4]), 5])  )
##    print result
##    print result.simplify()

##    result = do_multiply( Product([1, 2]), Sum( [Sum([3, 4]), 5])  )
##    print result
##
##    result = do_multiply( Product([1, 2]), Product( [Sum([3, 4]), 5])  )
##    print result

##    print is_flat(Sum([1, Product([2, 3])]))
##    print Sum([1, Product([2, 3])]).simplify()

##    print Sum([2, Product([3, Product([8, Sum([3, 12]), 5])]) ]).simplify()
    print Product([2, Product([3, Product([8, Sum([3, 12]), 5])]) ]).simplify()
